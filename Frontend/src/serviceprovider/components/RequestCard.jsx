...
const RequestCard = ({ request, onAction }) => {
  ...
  return (
    <>
      <div className="bg-white p-6 rounded-3xl border border-slate-100 hover:border-slate-200 hover:shadow-lg transition-all group overflow-hidden relative">
          ...
          <div className="flex items-start gap-4">
            ...
            >
              {request.userId?.profileImage ? (
                <img 
                  src={`/${request.userId.profileImage.replace(/\\/g, '/')}`} 
            ...
            <div className="flex items-center gap-2 ml-auto lg:ml-0">
              {request.status === 'Pending' ? (
                ...
                <div className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold border ${statusColors[request.status]}`}>
                  {request.status === 'Confirmed' ? <CheckCircle size={14} /> : <XCircle size={14} />}
                  {request.status}
                </div>
              )}
            </div>
